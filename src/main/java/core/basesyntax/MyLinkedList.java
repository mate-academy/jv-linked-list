...
public class MyLinkedList<T> implements MyLinkedListInterface<T> {
    private static final String OUT_OF_BOUND = "Index is not valid!";
    private int size;
    private Node<T> head;
    private Node<T> tail;
    ...
    public void add(T value) {
        Node<T> node = new Node<>(tail, value, head);
        if (size == 0) {
            head = node;
        } else {
            tail.next = node;
    ...
    private void deletingAnElement(Node<T> removedElement) {
        if (size == 1) {
            head = null;
            tail = null;
